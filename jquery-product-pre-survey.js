...
$(function() {
  ...
  */
  if ( affinity_status == 'inactive' || rowChk > 10 ) {
    /*
  ...
  $(qnumArr).each(function(a,i) {
    ...
    */
	...
	$.ajax ({
	  'type': 'post',
	  'url': 'adminUpdateAjax.iphtml',
	  'data': { item: item, type: 'smAffinity', question_num: i, remQ: 1 },
	  'dataType': 'html'
	}).done( function ( data ) {
	    ...
	    alert('unable to delete q'+i+' (' + status + ': ' + errorThrown + ')' );
        });
      }
    });
    /*
  ...
  */
  $('#add-question-smAffinity').click( function() {
	...
	*/
	...
	$('#q'+qnum$).on('click', '#delete-question-q'+qnum$, function() {
	    ...
	    $.ajax ({
	      'type': 'post',
              'url': 'adminUpdateAjax.iphtml',
              'data': { item: item, type: 'smAffinity', question_num: qnum$, remQ: 1 },
              'dataType': 'html'
	    }).done( function ( data ) {
	      $('#surveyContainer').html(data);
	      var qnum = (function () {
		...
		reloadCopyQuestion(item,inc);
	      ...
	      alert('unable to delete q'+qnum$+' (' + status + ': ' + errorThrown + ')' );
            });
          }
        });
	/*
	...
	*/
        ...
        $('#add-q-feedback').html( 'An error occured adding q'+qnum$+'(' + status + ': ' + errorThrown + ') - please consult a developer.' );
      });
    }
  });
});
